...
class QuestMasterCLI {
  ...
  /// Returns true if the input was a command (handled), false if it should
  /// be treated as a player action.
  ...
  Future<void> _processAction(String playerInput) async {
    ...
    // Get AI response (reusing MockAIService)
    final response = await _aiService.narrate(prompt: prompt);

    CliUI.clearThinking();

    // Create event (reusing GameEvent model)
  ...
  void _handleEquip(String itemName) {
    ...
    // Check if equipping grants vision
  ...
  void _applyStateChanges(Map<String, dynamic> changes) {
    if (changes.containsKey('itemGained')) {
      ...
      // Create a simple item from the AI response
      final item = GameItem(
        id: _uuid.v4(),
        name: itemName,
        type: ItemType.misc,
    ...
    if (changes.containsKey('combat')) {
      // Simulate taking some damage in combat
